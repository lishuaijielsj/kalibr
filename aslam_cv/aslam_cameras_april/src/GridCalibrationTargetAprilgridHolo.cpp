...
///        tagCols:    number of tags in x-dir = 1 (gridCols = 2*tagCols)
...
///   ^      0-------1
///   |-->x
GridCalibrationTargetAprilgridHolo::GridCalibrationTargetAprilgridHolo(
    size_t tagRows, size_t tagCols, std::string tagDatabase_file,
    const AprilgridOptionsHolo &options)
    : GridCalibrationTargetBase(2 * tagRows, 2 * tagCols),  //4 points per tag
      _options(options),
      _tagCodes(AprilTags::tagCodes36h11) {
  SM_ASSERT_TRUE(Exception, tagDatabase_file.size(), "tagDatabase_file name should not be NULL");
  SM_ASSERT_EQ(Exception, tagCols, 1, "tagCols should be 1 in GridCalibrationTargetAprilgridHolo");
  SM_ASSERT_GT(Exception, tagRows, 0, "tagRows should be larger than 0 in GridCalibrationTargetAprilgridHolo");

  // allocate memory for the grid points
  _points = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>::Zero(size(), 3);
  tagStatus.resize(tagRows, false);

  //initialize a normal grid (checkerboard and circlegrids)
  loadGridPoints(tagDatabase_file);
...
void GridCalibrationTargetAprilgridHolo::initialize()
{
...
///   ^      0-------1
///   |-->x
void GridCalibrationTargetAprilgridHolo::loadGridPoints(std::string tagDatabase_file) {
  //each tag has 4 corners
  //unsigned int numTags = size()/4;
  //unsigned int colsTags = _cols/2;

  std::ifstream ifile;
  ifile.open(tagDatabase_file.c_str());
  if(!ifile.is_open())
  {
      SM_ASSERT_TRUE(Exception, false, "can not open tag database: " + tagDatabase_file);
  }

  size_t num = 0;
  ifile.read((char*) &num, sizeof(size_t));

  for(size_t i = 0; i < num; i++){
      ::std::int32_t tagId;
      ifile.read((char*) &tagId, sizeof(::std::int32_t));
      //std::cerr<< "tagId: "<< tagId<< std::endl;
      Eigen::Matrix<double, 1, 3> center;
      //std::cerr<< "tag Center: \n";
      loadPoint(ifile, center);

      // index of bottom left corner
      size_t lb_index = 4 * tagId;
      SM_ASSERT_LT(Exception, lb_index, size(), "the tag Id larger out of range, please check the tag number \
                  that you want to use and the range of your tag database");

      // in our database each tag is stored by
      // 3------2
      // |      |
      // 0------1
      // here we need
      ...
      // |      |
      // 0------1
      size_t index[4] = {lb_index, lb_index + 1, lb_index + _cols + 1, lb_index + _cols};

      //std::cerr<< "\ntag Corners: \n";
      for(size_t j = 0; j < 4; j++){
          Eigen::Matrix<double, 1, 3> corner_j;
          loadPoint(ifile, corner_j);
          _points.row(index[j]) = corner_j;
          //std::cerr<< std::endl;
      }

      tagStatus.at(tagId) = true;
  ...
  for(size_t i = 0; i < size() / 4; i++)
  {
      if(tagStatus.at(i))
      {
          //std::cerr<< "tag: "<< i<< "is OK\n";
          for(size_t j = 0; j < 4; j++)
          {
              std::cerr<< _points.row(i * 4 + j)(0)<< " "
                       << _points.row(i * 4 + j)(1)<< " "
                       << _points.row(i * 4 + j)(2)<< std::endl;
...
void GridCalibrationTargetAprilgridHolo::loadPoint(
        std::ifstream& ifile, Eigen::Matrix<double, 1, 3>& pt){
    for(size_t i = 0; i < 3; i++){
        ifile.read((char*) &pt(i), sizeof(double));
        //std::cerr<< pt(i)<< " ";
    }
}

/// \brief extract the calibration target points from an image and write to an observation
bool GridCalibrationTargetAprilgridHolo::computeObservation(
  ...
  for (iter = detections.begin(); iter != detections.end();) {
    ...
    if (iter->good != 1)
    {
      //std::cerr<< "removed "<< iter->id<< "due to not good\n";
      remove |= true;
    }
    ...
    {
      //std::cerr<< "removed "<< iter->id<< "due to out of range\n";
      remove |= true;
    }

    // also remove if the tag is invalid
    if(!remove && !tagStatus.at(iter->id))
    {
        //std::cerr<< "removed "<< iter->id<< "due to no 3d\n";
        remove |= true;
    }
  ...
  {
      //LSJ
      size_t win_size=(size_t)(cv::norm(tagCorners.row(0) - tagCorners.row(1))/16);
      win_size = win_size>=2 ? win_size : 2;
    cv::cornerSubPix(
        image, tagCorners, cv::Size(win_size, win_size), cv::Size(-1, -1),
...
BOOST_CLASS_EXPORT_IMPLEMENT(aslam::cameras::GridCalibrationTargetAprilgridHolo);
BOOST_CLASS_EXPORT_IMPLEMENT(aslam::cameras::GridCalibrationTargetAprilgridHolo::AprilgridOptionsHolo);
